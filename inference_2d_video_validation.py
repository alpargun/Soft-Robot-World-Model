...
def main():
    ...
    # Create directory to save all 12 generated videos
    OUTPUT_DIR = f"validation_videos_{TIMESTAMP}"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    print(f"Videos will be saved to directory: {OUTPUT_DIR}")
    ...
    print(f"\nRetrieved {len(validation_indices)} seeded validation indices: {validation_indices}\n")

    # Loop through every single validation case
    for idx_num, original_dataset_idx in enumerate(validation_indices):
        print(f"--- Generating Video {idx_num + 1}/{len(validation_indices)} (Dataset Index: {original_dataset_idx}) ---")
        
        # THE FIX: Load TWO DISTINCT runs to give BatchNorm actual variance!
        # (Comment preserved, but we are using Batch Size 1 since we have GroupNorm now)
        test_dataset = Subset(full_dataset, indices=[original_dataset_idx]) 
        dataloader = DataLoader(test_dataset, batch_size=1, shuffle=False)
        
        batch = next(iter(dataloader))
        real_videos = batch["video"].to(device)    # Shape will be [1, 60, 4, 3, 128, 128]
        pressures = batch["pressures"].to(device)  # Shape will be [1, 60, 3]
        ...
        # ==========================================
        OUTPUT_VIDEO_PATH = os.path.join(OUTPUT_DIR, f"Validation_Case_Idx_{original_dataset_idx}.mp4")
    ...
    print(f"\nAll 12 evaluation videos successfully saved to: {OUTPUT_DIR}")
